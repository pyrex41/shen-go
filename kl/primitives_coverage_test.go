...
// `str` primitive (PrimStr) and ObjString (GoString) — matching shen-cl,
// shen-rust and ShenScript. Integral-valued floats keep printing without a
// decimal, as the kernel expects.
func TestFloatShortestFormIssue11(t *testing.T) {
	var ctx ControlFlow

	cases := []struct{ input, want string }{
		// reference targets from the task / sibling ports
		{"(/ 10 4)", "2.5"},
		{"(+ 3.5 1.25)", "4.75"},
	...
	for _, c := range strCases {
		if got := ObjString(evalString(&ctx, c.input)); got != c.want {
			t.Errorf("str %s: got %q, want %q", c.input, got, c.want)
		}
	}
}
