{
  ...
  outputs = { nixpkgs, ... }: let systems = [ "aarch64-darwin" "x86_64-darwin" "aarch64-linux" "x86_64-linux" ]; each = f: nixpkgs.lib.genAttrs systems (system: f nixpkgs.legacyPackages.${system}); in {
    packages = each (pkgs: { toolchain = pkgs.buildEnv { name = "shen-go-toolchain"; paths = [ pkgs.go ]; }; default = pkgs.buildEnv { name = "shen-go-toolchain"; paths = [ pkgs.go ]; }; });
    devShells = each (pkgs: { default = pkgs.mkShell { packages = [ pkgs.go ]; }; });
  };
}
